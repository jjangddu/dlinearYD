...
###############################
# Dataset 및 모델 정의 (출력 y는 단일 채널: Close)
###############################
class StockDataset(Dataset):
    def __init__(self, data, seq_len, pred_len, target_idx=0):
        ...
        self.target_idx = target_idx  # 예측 목표, 여기서는 Close (index 0)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        kernel_size = configs.kernel_size  # configs에서 kernel_size 전달받음
        self.decompsition = series_decomp(kernel_size)
        self.individual = configs.individual
        self.channels = configs.enc_in  # 입력 피처 수 (여기서는 3)
        if self.individual:
    ...
    def forward(self, x):
        ...
        if self.individual:
            ...
            x_out = seasonal_output + trend_output  # shape: (batch, channels, pred_len)
        else:
            x_out = self.Linear_Seasonal(seasonal_init) + self.Linear_Trend(
                trend_init)  # shape: (batch, channels, pred_len)

        # 올바른 채널(예를 들어 Close 값이 0번 인덱스라고 가정) 선택:
        # x_out[:, 0, :]는 shape (batch, pred_len)이고, 이를 unsqueeze하면 (batch, pred_len, 1)가 됨.
        x_out = x_out[:, 0, :].unsqueeze(-1)
        return x_out  # 최종 출력: (batch, pred_len, 1)
...
class CustomLoss(nn.Module):
    ...
    def forward(self, x, output, target):
        """
        x: 입력 시퀀스, shape: (batch, seq_len, features)
           여기서 마지막 시점의 0번 채널이 Close
        output: 모델 예측, shape: (batch, pred_len, 1)
        target: 실제 값, shape: (batch, pred_len, 1)
        """
        mse = self.mse_loss(output, target)

        # 입력 시퀀스의 마지막 Close 값을 기준으로 변화량 계산
        # x[:, -1, 0]의 shape: (batch,)
        last_value = x[:, -1, 0].unsqueeze(-1).unsqueeze(-1)  # (batch, 1, 1)
        true_return = target - last_value  # (batch, pred_len, 1)
        pred_return = output - last_value  # (batch, pred_len, 1)

        # 패널티: 실제 변화(true_return)의 부호와 예측 변화(pred_return)의 부호가 다르면 패널티를 부여
        # -pred_return * sign(true_return)은, 예를 들어 true_return > 0 (즉, 상승 기대)인데 pred_return < 0이면 양수가 됨
        penalty = torch.relu(-pred_return * torch.sign(true_return)).mean()

        loss = mse + self.penalty_weight * penalty
        return loss
...
def process_ticker(ticker, start_date, end_date):
    ...
    # 추가: High_Close_Ratio와 Low_Close_Ratio 피처 추가
    ...
    # 단, 여기서는 예측 목표(y)는 단일 채널(Close)로 하므로,
    # 입력 피처로는 ['Close', 'High_Close_Ratio', 'Low_Close_Ratio']를 사용하되,
    # 출력 y는 'Close' (인덱스 0)를 사용하도록 Dataset이 구성됨.
    features_for_model = ['Close', 'High_Close_Ratio', 'Low_Close_Ratio']
    X = processed_data[features_for_model].values
    scaler = MinMaxScaler(feature_range=(0, 1))
    X_scaled = scaler.fit_transform(X)

    # Close 전용 scaler 생성 (예측 역변환용)
    ...
    train_window = timedelta(days=365 * 2)
    val_window = timedelta(days=180)
    ...
    criterion = CustomLoss(penalty_weight=1.0)
    ...
    for i, window in enumerate(windows):
        ...
        train_dataset = StockDataset(train_data, seq_len, pred_len, target_idx=0)
        val_dataset = StockDataset(val_data, seq_len, pred_len, target_idx=0)
    ...
    test_dataset = StockDataset(test_data, seq_len, pred_len, target_idx=0)
    ...
    output_file2 = f"{ticker}_prediction_27.json"
...
save_stocks_file("stocks.json", ["AAPL", "NVDA"])
